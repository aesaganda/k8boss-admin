...
export default function Users() {
  ...
  return (
    <>
      <PageHeader
        title="Users"
        subtitle="Local accounts, and the identities that signed in through a configured provider. Identity providers is where the providers themselves are configured."
        actions={<Button variant="primary" onClick={() => setEditing(EMPTY_FORM)}>Add local user</Button>}
      />
      <SectionHeader
        title="Console accounts"
        description="Local users, plus every identity from a configured provider that has signed in at least once — directory and single sign-on accounts appear after their first successful login, never before."
      />
