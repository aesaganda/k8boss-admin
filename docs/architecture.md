...
| `services/quota.py` | §29 whether the next workload is admitted, and which bound refuses it. Arithmetic over `spec.hard` minus `status.used` plus a LimitRange's `defaultRequest`, answered before a manifest exists. Keeps two refusals apart that a 403 makes look alike: no headroom, and the compulsory-resource rule — a quota bounding `requests.cpu` refuses every container that omits it **at any level of usage**, and the fix is a LimitRange the message never names. `unknown` is a verdict: an unwritten `status.used` is not zero, a scoped quota's applicability to a non-existent pod is not guessed, and an unreadable LimitRange listing suppresses the "must specify" check rather than producing a refusal a default might have discharged |
| `services/scheduling.py` | §31 why a pod is Pending. Splits the phase in two before anything else — a pod with `spec.nodeName` set has been *placed* and is waiting on the kubelet, and showing it node capacity sends an operator to audit a fleet over one failed image pull. Quotes the scheduler's own `FailedScheduling` message rather than deriving one, with its age beside it, because the message is a **snapshot**: a node added since does not rewrite it. `scheduler: null` is "no such event is readable" — events age out of etcd within the hour — and never "the scheduler is content". The per-node table only ever **rules a node out**: there is no `fits`, because the scheduler also weighs affinity, topology spread, volume zone, extended resources and every plugin the cluster runs, none of which is evaluated here — and `capacity_checked: false` marks the rows that reached `no_reason_found` by an even shorter path. An unbound `WaitForFirstConsumer` claim is reported as the symptom it is, not the cause |
| `services/tls.py` | §32 the certificate each exposure points at, decoded. Answers the two facts no Kubernetes API reports — when it expires, and whether its subject alternative names cover the hostname the exposure serves — from Secrets, from a Route's inline `spec.tls.certificate`, and from Gateway listener `certificateRefs`. **One entry per certificate, not per object**: an Ingress with two `spec.tls[]` blocks has two, and §13's row reporting the first is a summary that would say the wrong certificate covers a host. `state: unknown` is a Secret that could not be read and never an exposure with no certificate; `expires_in_seconds` is `null` for that and negative — never clamped — for an expired one, because `0` here means *expires this second*. Coverage is tri-state and matched against SANs with RFC 6125 wildcard rules, because every browser ignores the Common Name. Verifies nothing: no trust store, no chain, no revocation, no handshake — and reads exactly one key, `tls.crt`, of `kubernetes.io/tls` Secrets only |
| `services/disruption.py` | §28 what a PodDisruptionBudget actually covers. The typed row is a pure shaper; this module owns the correlation that needs a pod listing — `selected_pods` (**`0` is the finding, `null` is a listing that did not answer**), and the overlap index: Kubernetes refuses the eviction outright for a pod covered by two budgets, whatever either budget's `disruptionsAllowed` says, and nothing on either object mentions the other. Uses the **tri-state** `label_selector_matches`, whose `None` reaches the response as `undecidable_pods` rather than being flattened to `False` — which would manufacture the "protects nothing" sentence this module exists to make trustworthy. It was written against a second matcher that did flatten it; that duplicate is gone (contract §28.5) |
| `admin/rbac_grants.py` | §30 adding or removing one subject on one RoleBinding. The write is §4's, unchanged; the module is the plan in front of it, because an RBAC diff is a name appearing in a list and what that name can do is in a different object. Resolves the role and reports five capabilities — a wildcard, `create rolebindings` (the grantee can grant themselves the rest), `create pods/exec` (reads every Secret mounted here **without a Secrets rule**), `impersonate`, direct Secret reads — and keeps three states apart that a rule count would collapse: `present`, `absent` (**the API server accepts a binding to a role that does not exist**, and it starts granting when one appears) and `unreadable`, where `rule_count` is `null` and never `0`. On a revoke it lists what *else* names the subject, with `cluster_bindings: null` when that listing was refused, so `applied: true` is never read as "revoked". Refuses rather than guesses where a guess would be a false report: two bindings sharing a roleRef, a binding-name collision. Patches, never deletes |
...
| `k8s/impersonation.py` | ADR-0007's exception, whole. Decides whether a request acts as the signed-in operator (`Impersonate-User` / `Impersonate-Group`) or as the console's ServiceAccount, and **raises rather than falling back** — a read served as the console on a cluster that impersonates would show somebody data their own RBAC forbids. Enforces the ADR's conditions in one place: `IMPERSONATION_SOURCES` sessions only (OIDC and OpenShift — the two whose username the API server could have derived itself), an absent groups claim refuses where an empty one does not, the issuer's username verbatim rather than the console's casefolded key, groups as repeated headers because Go's `http.Header` does not split a comma-joined one, and no invented `Impersonate-Uid`. `as_service_account()` is the only suppression and its call sites are enumerated by a test |
...
| `identity/sso.py` | What the four single sign-on providers share — the identity shape they reduce to, the tri-state group rule, the allowlist that fails closed, the TLS trust configuration — and the registry the generic `/api/auth/{provider}` routes drive them through. A registry of modules rather than a class hierarchy, because the interesting content of each provider is its *argument* for why an assertion is trustworthy, and that belongs beside the code it justifies |
...
| `identity/oauth.py` | A plain OAuth 2.0 server with no ID token. Identity comes from a userinfo read with the freshly minted token, which makes the verified TLS connection the whole trust argument — stated in the module rather than assumed |
| `identity/openshift.py` | The cluster's own OAuth server. One setting (the API server) yields both endpoints and the `users/~` read; the access token is spent once and dropped, so the console stores no cluster credential |
| `identity/saml.py` | SAML 2.0. Everything read comes out of the subtree the signature check returned and never out of the document as posted — the structural defence against XML Signature Wrapping, which is a property of the code's shape rather than a check |
| `identity/handshake.py` | The sealed, short-lived cookie carrying one sign-in across the redirect. One per provider, and its `SameSite` is `Lax` for the redirect flows and `None` for SAML's cross-site POST — never `Strict` like the session cookie, and the module says why each |
...
| `components/ImportYamlDialog.jsx` | The **one** create dialog: the masthead's `+`, and every listing's `Create <Kind>…`. It takes no group/version/plural — it resolves where to POST from the document's own `apiVersion` and `kind` against §4's catalog, so a second copy of that mapping cannot exist |
| `components/templates.js` | The starters each create button seeds, keyed by `apiVersion` and `kind`, and the labelled skeleton for kinds it ships none for. In `components/` rather than `pages/` because the dialog needs it and a components → pages import runs the wrong way |
| `app/yaml_dialect.py` | §11.12, ADR-0009 and ADR-0010. **The backend's only YAML.** PyYAML's `SafeLoader` plus `y`/`Y`/`n`/`N` as booleans — the single-letter half of YAML 1.1's boolean type that PyYAML declines and `kubectl` does not — with a dumper carrying the same resolver, so the text the editor is seeded with means what a save of it will send. Two scanners, one reading: PyYAML's for anything an operator wrote, because it names the character it refused, and libyaml's for `deploy/olm/`'s 1.1 MiB |
| `components/clusterYaml.js` | §11.12, ADR-0009. **The browser's only YAML.** js-yaml with three scalar resolvers transcribed from PyYAML's, so a manifest is read and re-serialised the way `admin/apply.py` sends it — the form view's controls, `unrepresented()` and `localIssues()` are then lenses onto the object being written rather than onto a second reading of it. Also parses a second time with js-yaml's own YAML 1.2 schema and reports where the two differ, which is a warning about the *document* and never a gate — and a parse rather than a pattern over the source, because only a parser can tell a plain `off` from one inside a `|` block |
| `components/ObjectForm.jsx`, `objectFormModel.js` | §11.9's form view. `objectFormModel.js` holds the field models and the path lenses (no JSX, so a test can reach the model without mounting anything); the `.jsx` half renders them. The model is **not** called `objectForm.js`: two modules whose names differ only in case resolve to the same file on macOS and Windows, and `import ObjectForm from './ObjectForm'` silently picked up the model — a build that only fails off Linux CI. `unrepresented()` is what lets the form name, by path, every field it is not showing |
