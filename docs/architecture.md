...
| `components/yamlDivergence.js` | §11.12. The browser parses with js-yaml (YAML 1.2) and `admin/apply.py` with PyYAML (YAML 1.1); this parses a second time against a schema mirroring PyYAML's scalar resolvers and reports where the two readings differ. A warning, never a gate — and a second parse rather than a pattern over the source, because only a parser can tell a plain `off` from one inside a `|` block |
