...
| `services/disruption.py` | §28 what a PodDisruptionBudget actually covers. The typed row is a pure shaper; this module owns the correlation that needs a pod listing — `selected_pods` (**`0` is the finding, `null` is a listing that did not answer**), and the overlap index: Kubernetes refuses the eviction outright for a pod covered by two budgets, whatever either budget's `disruptionsAllowed` says, and nothing on either object mentions the other. Uses the **tri-state** `label_selector_matches` rather than `workloads.selector_matches`, whose resolve-to-False would manufacture the "protects nothing" sentence this module exists to make trustworthy |
