...
| `backend/app/k8s/` | Per-request cluster context, auth strategy (bearer tokens and §34's X.509 client certificates), client manager, ADR-0007's impersonation decision (`impersonation.py`) — whether a call acts as the signed-in operator or as the console, refusing rather than falling back — and §34's onboarding: reading this machine's kubeconfig and classifying every context (`kubeconfig.py`), and copying one into the registry (`adoption.py`) | `docs/architecture.md` §2, `docs/adr-0007-impersonation.md`, `docs/adr-0012-kubeconfig-onboarding.md` |
...
| `backend/app/identity/` | Local password hashing, opaque sessions — listed and revoked one by one, with where each was opened from (`service.py`) — LDAP search-and-bind, sign-in throttling, four single sign-on providers behind one registry (`sso.py`) and one pair of routes: OIDC (`oidc.py`), a plain OAuth 2.0 server (`oauth.py`), the cluster's own OAuth server (`openshift.py`), and SAML 2.0 (`saml.py`), and what this deployment actually has configured — one row per kind in the database, or the environment behind it, resolved in one place (`provider_config.py`, `provider_store.py`, `inventory.py`; ADR-0011) | `docs/api-contract.md` §12 |
| `backend/tests/` | pytest on SQLite. The fake Kubernetes client **raises** on an unstubbed call | — |
| `frontend/src/` | React 19 / Vite / PatternFly 6 SPA. `components/ImportYamlDialog.jsx` is the **one** create dialog behind every `Create <Kind>…` button and the masthead's `+`; `components/templates.js` holds the starters, and the skeleton for the kinds it ships none for; `pages/topologyGraph.js` is §11.13's join — pure, no I/O — and holds the one rule that decides what the topology view is allowed to draw; `components/clusterYaml.js` is the browser's only YAML — js-yaml carrying `yaml_dialect.py`'s scalar resolvers, so the console reads a manifest the way it sends it (ADR-0009), and warns where a YAML 1.2 reader would read it otherwise | `docs/api-contract.md` §11 (§11.4, §11.9–§11.13) |
...
| `docs/adr-0012-kubeconfig-onboarding.md` | **Why the console reads a kubeconfig at all, when `client.py` says it never does.** The distinction that makes both true: discovery lists and an import *copies* — every client is still built from a `Cluster` row, and the file is never read again. Also why `exec` contexts are refused rather than run, why a loopback address in a container is a stated concern rather than a silent URL rewrite, why startup adoption takes one local cluster and never a second or a remote one, and what a kind cluster's `cluster-admin` certificate costs next to a scoped ServiceAccount token |
| `docs/adr-0011-database-backed-identity-providers.md` | **Why §12.4's refusal was narrowed rather than dropped.** Identity providers are configurable from the console, one row per kind, with the environment as the fallback — which of §12.4's four costs are paid, why the subject-collision cost does not apply when a second provider of a kind cannot exist, and why `resolve()` may never be cached |
